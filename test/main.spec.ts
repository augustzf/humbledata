...
import * as deniro from './deniro.json'
...
describe("Aggregate functions", () => {
    ...
    it("should find max for a field", () => {
        const f = new Builder(deniro).build()
        const sum = f.max("score")
...
describe("Filter functions", () => {
    ...
    it("should filter on ==", () => {
        const f = new Builder(deniro).build()
        const movie = f.where("title", "==", "Taxi Driver")
        expect(movie.value("score")).to.equal(99)
    })
})
